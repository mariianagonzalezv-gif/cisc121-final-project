...
def bubble_sort_steps(arr):
  ...
  steps.append(f"🫧 starting list: {a}\n")
  ...
  while swapped == True:
    ...
    for i in range(n-1):
      ...
      if a[i] > a[i+1]:
        steps.append(f"🫧 compare {a[i]} and {a[i+1]}, since {a[i]} is greater than {a[i+1]} we swap!")
        swapped = True
        a[i], a[i+1] = a[i+1], a[i]
      else:
        steps.append(f"🫧 compare {a[i]} and {a[i+1]}, since {a[i]} is not greater than {a[i+1]} we don't swap!")

      steps.append(f"🫧 new list: {a}\n")

  steps.append(f"🫧 finished! sorted list: {a}")
  return a, "\n".join(steps)
...
with gr.Blocks() as demo:
  ...
  #click to sort
  sort_button = gr.Button("🫧 Click to sort! 🫧", variant="primary")
